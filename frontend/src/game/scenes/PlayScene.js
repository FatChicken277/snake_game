...
export default class PlayScene extends Scene {
  ...
  create () {
    ...
    this.input.on('pointermove', pointer => {
      if (this.alive == true) {
        this.physics.moveTo(this.head, pointer.worldX, pointer.worldY, 250)
      }
    }, this)
    ...
    this.time.addEvent({ delay: 5000, callback: this.generateFood, args: [RED_APPLE], callbackScope: this, loop: true })
  ...
  generateFood (type) {
    let add = type === GREEN_APPLE || Boolean(Math.random() <= 0.5)

    if (this.food.countActive() > 19 || !add)
      return
  ...
  generateObstacle() {
    ...
    let angle = Math.random() <= 0.5 ? VERTICAL_OBSTACLE : HORIZONTAL_OBSTACLE

    let obstacle = this.obstacles.create(randomX, randomY, angle)
